...
describe('CommunityCollectionAssetDialogContent', () => {
  test('renders the asset detail sections and calls the navigation handlers', async () => {
    ...
    expect(view.getByRole('heading', { name: 'JSON Metadata' })).toBeTruthy()
    expect(view.getByRole('link').getAttribute('href')).toBe('https://example.com/asset-beta.json')
    expect(view.getByRole('button', { name: 'Copy JSON' })).toBeTruthy()

    fireEvent.click(view.getByRole('button', { name: 'Expand' }))
    expect(view.getByText('"name"')).toBeTruthy()
    expect(view.getByText('"Perk #2"')).toBeTruthy()
